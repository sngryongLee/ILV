...
from lightning.network_full import Network
...
class system(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for key, value in scalar_stats.items():
            prog_bar = True if key in ['psnr','vol_psnr'] else False
            self.log(f'train/{key}', value, prog_bar=prog_bar)

        self.log('lr',self.trainer.optimizers[0].param_groups[0]['lr'])

        if 0 == self.trainer.global_step % 200:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        scalar_stats = {k: v.detach() for k, v in scalar_stats.items()}
    ...
    def vis_results(self, output, batch, prex):
        ...
        for key, value in output_vis.items():
            if isinstance(self.logger, TensorBoardLogger):
                B,h,w = value.shape[:3]
                value = value.reshape(1,B*h,w,1).transpose(0,3,1,2)
